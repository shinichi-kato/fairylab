...
function reducer(state,action){
  switch(action.type){
    ...
    case 'LocalSave':{
      const newParts=JSON.parse(localStorage.getItem('bot.parts'));
      const cell = {
        name:state.name,
        type:state.type,
      ...
      localStorage.setItem('bot.parts',JSON.stringify(newParts));
      localStorage.setItem(`bot.dict.${state.name}`,JSON.stringify(action.dict));
...
export default function PartEditor(props){
  ...
  const [state,dispatch] = useReducer(reducer,initialState(props.state));
      ...
      <Grid container spacing={1}>
        ...
        <Grid item xs={12}>
          <Button
            ...
            variant="contained"
            color="primary"
            onClick={e=>dispatch({type:'LocalSave',dict:dict})}>
            保存
          </Button>
        </Grid>
      </Grid>
